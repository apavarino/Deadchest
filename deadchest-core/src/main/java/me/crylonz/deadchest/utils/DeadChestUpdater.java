...
public class DeadChestUpdater {
    ...
    private static final String VERSION_FILENAME = "fileName";
    // Path to GET
     ...
     */
    public DeadChestUpdater(Plugin plugin, int id, File file, UpdateType type, boolean announce) {
     ...
     */
    public DeadChestUpdater(Plugin plugin, int id, File file, UpdateType type, UpdateCallback callback) {
    ...
    public DeadChestUpdater(Plugin plugin, int id, File file, UpdateType type, UpdateCallback callback, boolean announce) {
        ...
        try {
            this.url = new URL(DeadChestUpdater.HOST + DeadChestUpdater.QUERY + this.id);
    ...
    private void downloadFile() {
        ...
        try {
            ...
            long downloaded = 0;
            while ((count = in.read(data, 0, DeadChestUpdater.BYTE_SIZE)) != -1) {
                downloaded += count;
        ...
        try {
            ...
            while (e.hasMoreElements()) {
                ...
                if (!entry.isDirectory()) {
                    ...
                    int b;
                    final byte[] buffer = new byte[DeadChestUpdater.BYTE_SIZE];
                    final FileOutputStream fos = new FileOutputStream(destinationFilePath);
                    final BufferedOutputStream bos = new BufferedOutputStream(fos, DeadChestUpdater.BYTE_SIZE);
                    while ((b = bis.read(buffer, 0, DeadChestUpdater.BYTE_SIZE)) != -1) {
    ...
    private boolean hasTag(String version) {
        for (final String string : DeadChestUpdater.NO_UPDATE_TAG) {
    ...
    private boolean read() {
        try {
            ...
            conn.addRequestProperty("User-Agent", DeadChestUpdater.USER_AGENT);
            ...
            this.versionName = (String) latestUpdate.get(DeadChestUpdater.TITLE_VALUE);
            this.versionLink = (String) latestUpdate.get(DeadChestUpdater.LINK_VALUE);
            this.versionType = (String) latestUpdate.get(DeadChestUpdater.TYPE_VALUE);
            this.versionType = (String) latestUpdate.get(DeadChestUpdater.TYPE_VALUE);
            this.versionGameVersion = (String) latestUpdate.get(DeadChestUpdater.VERSION_VALUE);
            this.versionFileName = (String) latestUpdate.get(DeadChestUpdater.VERSION_FILENAME);
    ...
    public interface UpdateCallback {
         ...
         */
        void onFinish(DeadChestUpdater updater);
    }
